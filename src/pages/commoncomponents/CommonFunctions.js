...
export const getQuantitydetails = async (cartData) => {
    ...
    if (itemsIds.length > 0) {
        try {
            const response = await fetch('http://localhost:5001/api/v1/vibe-cart/app/orders/sku/total-quantity', { method: "POST", headers: { 'content-type': 'application/json', 'Accept': 'application/json' }, body: JSON.stringify(itemsIds) });
