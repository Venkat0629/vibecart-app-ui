...
const ProductDetailPage = () => {
    ...
    const fetchProductDetails = async () => {
      try {
        ...
        if (productId <1000900) {
          // Handle itemId
          url = `http://localhost:4001/vibecart/ecom/items/item/${productId}`;
          response = await axios.get(url);
        } else {
          // Handle skuId
          url = `http://localhost:4001/vibecart/ecom/products/product/sku-id/${productId}`;
          response = await axios.get(url);
        }

        const productData = response.data;
        dispatch(setSelectedProduct(productData));
        if (url === `http://localhost:4001/vibecart/ecom/items/item/${productId}`) {
  ...
  useEffect(() => {
    const fetchSkuDetails = async () => {
      if (selectedColor && selectedSize && product) {
        try {
          const response = await axios.get(`http://localhost:4001/vibecart/ecom/products/product/item-id/${product.itemID}`, {
          ...
          const stockResponse = await axios.get(`http://localhost:4001/vibe-cart/scm/inventory/quantity-by-sku`, {
            params: { sku: skuID }
          });
  ...
  useEffect(() => {
    const fetchOffersByItemID = async () => {
      if (product && product.itemID) {
        try {
          const response = await axios.get(`http://localhost:4001/api/v1/vibe-cart/offers/item/${product.itemID}`);
  ...
  useEffect(() => {
    const fetchOffersBySKU = async () => {
      if (skuID) {
        try {
          const response = await axios.get(`http://localhost:4001/api/v1/vibe-cart/offers/sku/${skuID}`);
          const offers = response.data || [];
  ...
  const fetchExpectedDeliveryDate = async () => {
    if (skuID && zipcode.length === 6) {
      try {
        const response = await axios.get(`http://localhost:4001/vibe-cart/scm/inventory/expected-delivery-date`, {
